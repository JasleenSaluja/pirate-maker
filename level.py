...
class Level:
		...
		for layer_name, layer in grid.items():
			for pos, data in layer.items():
				...
				match data:
					...
					case 8: 
						Tooth(asset_dict['tooth'], pos, [self.all_sprites, self.damage_sprites,self.killable_sprites], self.collision_sprites) #change
					case 9: 
						Shell(
							orientation = 'left', 
  ...
  # killing the tooth if the player jumps on it
	def tooth_kill(self):  #change
		...
		if tooth_sprites:
			for tooth in tooth_sprites:
				self.tooth_center=tooth.rect.centery
